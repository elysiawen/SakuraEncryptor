...
def _walk_and_encrypt(src_dir: Path, dst_dir: Path, password: str) -> None:
    ...
    for root, dirs, files in os.walk(src_dir):
        ...
        for fname in files:
            ...
            # Encrypt the file name (without extension) and append .skvmod
            enc_fname = encrypt_name(fname, key) + SKE_EXT
            dst_file = enc_root / enc_fname

            click.echo(f"  encrypting: {src_file.relative_to(src_dir)}")
    ...
    for root, dirs, files in os.walk(src_dir):
        ...
        for part in rel_root.parts:
            try:
                ...
                # If a folder name can't be decrypted, keep it as-is
        ...
        for fname in files:
            if not fname.endswith(SKE_EXT):
                click.echo(f"  skipping (not .skmod): {fname}")
                continue

            src_file = Path(root) / fname
            # Remove .skvmod extension, decrypt name
            ...
            click.echo(f"  decrypting: {src_file.relative_to(src_dir)}")
            try:
...
def encrypt(src: str, dst: str, password: str) -> None:
    ...
    if src_path.is_file():
        ...
        _walk_and_encrypt(src_path, dst_path, password)
    click.echo("Done.")
